...
import { newShip, refit, step, stepVitals, stepDrift, applyDamage, drainHull, stepJump, beginJump, arrivalFor, inBase, canDock, inHaven, inOutpost, shieldMax, WORLD, boundsOf, SHIELD_FLASH, SHOT_FLASH } from './shared/sim.js';
...
import { DUEL_KEY, startsAt, HOME_TO, COUNT as DUEL_COUNT,
         ...
         whyNotChallenge, stakeOf, challengeText, mayAim } from './shared/duel.js';
...
setInterval(() => {
  ...
  for (const [id, p] of players) {
    ...
    // `dry` is keyed on being in ANY instanced sector, and it now covers duels for a
    // reason of their own rather than by accident. In a claim it removes the rest
    // button — kill one, walk out, let 3.33% of the pool a second put the ship back.
    // In a duel it removes the STALEMATE: two evenly matched pilots in a 6,000px box
    // with regenerating shields have an obvious dominant line, which is to kite until
    // the five-minute wall and take the draw. Without regeneration the ship each of
    // them arrives in is all they get, so somebody has to commit.
