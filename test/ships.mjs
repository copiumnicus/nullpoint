...
console.log('\nshields, and the countdown to them');
{
  ...
  // REWRITTEN, and the reason is the game changing rather than the claim being
  // wrong. This said "inside a claim there is no countdown, because the shields are
  // not coming back" — true while `dry` existed. The designer had never asked for
  // that: it made a claim harder by removing a mechanic rather than by making the
  // fight harder, and it is gone. A claim counts down like everywhere else, and
  // what stops you resting in one is now that nothing in it ever breaks off.
